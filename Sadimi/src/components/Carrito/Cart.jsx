...
function Cart() {
    ...
    return (
        <aside className={`flex flex-col gap-4 p-4 fixed right-0 top-36 sm:top-24 rounded-l-xl min-w-96 z-40 h-[84svh] md:h-[620px] w-full md:w-[620px] bg-tercero shadow-xl overflow-y-auto ${cart ? "flex" : "hidden"}`}>
            ...
            <div className='flex h-full flex-col gap-4 overflow-y-auto rounded-xl'>
                {carrito.length == 0 ? (
                    <div className='flex h-full items-center justify-center'>
                        <h1 className='text-center text-2xl font-bold text-quinto'>No hay productos en el carrito</h1>
                    </div>
                ) :
            ...
            <div className='flex w-full items-center justify-center'>
                <button className='w-2/3 rounded-xl bg-primero p-2 px-6 text-2xl text-tercero'>Comprar</button>
            </div>
        </aside >
    );
}
