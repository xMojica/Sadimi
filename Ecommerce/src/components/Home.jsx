                ...
                <article key={product._id} className="flex h-[28rem] w-40 flex-col justify-center overflow-hidden rounded-xl border-2 bg-tercero p-3 shadow-lg hover:cursor-pointer md:w-60">
                    ...
                    <div className='mt-auto flex flex-row flex-nowrap gap-2'>
                        {product.colores && product.colores.map((color, index) => (
                            <span className='flex items-center justify-center rounded-[50%] border border-primero p-1'>
                                <svg className='hover:scale-125' key={index} width="16px" height="16px" viewBox="0 0 16 16" fill={color} stroke-width="2" xmlns="http://www.w3.org/2000/svg">
